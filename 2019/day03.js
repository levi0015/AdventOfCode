...
function day03() {
    ...
    for (let i = 0; i < day03Arr.length; i++) {
        let route = [];
        let coors = [0, 0];
        for (let j = 0; j < day03Arr[i].length; j++) {
            test:
            for (let k = 0; k < Number(day03Arr[i][j].slice(1)); k++) {
                switch (day03Arr[i][j][0]) {
                    case "U":
                        coors[1]++;
                        break;
                    case "D":
                ...
                for (let l = 0; l < route.length; l++) {
                    if (coors[0] == route[l][0] && coors[1] == route[l][1]) {
                        continue test;
                    }
                }

                route.push([coors[0], coors[1]]);
            }
        }
        routes.push(route);
    }
    ...
    for (let m = 0; m < routes.length; m++) {
        for (let n = (m + 1); n < routes.length; n++) {
            for (let o = 0; o < routes[m].length; o++) {
                for (let p = 0; p < routes[n].length; p++) {
                    if (routes[m][o][0] == routes[n][p][0] && routes[m][o][1] == routes[n][p][1]) {
                        intersections.push(routes[m][o])
                    }
                }
            }
        }
    ...
    for (let q = 0; q < intersections.length; q++) {
        let sum = 0;
        for (let r = 0; r < intersections[q].length; r++) {
            if (intersections[q][r] >= 0) {
                sum += intersections[q][r];
            } else {
                sum += (intersections[q][r] * -1);
            }
        }
        intersections[q] = sum;
    }
...
console.log(`Part one's answer is: 225`);
console.log(`Part two's answer is: 35194`);