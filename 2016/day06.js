...
function day06(partTwo) {
    let verticallyFiltered = [];

    let i = 0;
    let j = 0;
    while (j < day06Arr[i].length) {
        let column = "";
        while (i < day06Arr.length) {
            column += day06Arr[i][j];
            i++;
        }
        verticallyFiltered.push(column);
        j++;
        i = 0;
    }

    let answer = "";
    for (let k = 0; k < verticallyFiltered.length; k++) {
        if (!partTwo) {
            answer += maxChar(verticallyFiltered[k], true);
        } else {
            answer += maxChar(verticallyFiltered[k]);
        }
    }
