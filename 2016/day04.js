...
function day04() {
    ...
    test:
    for (let i = 0; i < day04Arr.length; i++) {
        let currRoom = [...day04Arr[i]];
        const ID = day04Arr[i].slice(-2)[0];
        const pass = day04Arr[i].slice(-2)[1];

        for (let j = 1; j < day04Arr[i].slice(0, -2).length; j++) {
            day04Arr[i][0] += day04Arr[i][j];
        }
        // console.log(day04Arr[i]);
        const strObject = countAmountOfEachCharacter(day04Arr[i][0]);
        let charCount = [];

        for (let k = 0; k < pass.length; k++) {
            const char = pass[k];
            for (const key in strObject) {
                if (Object.hasOwnProperty.call(strObject, key) && (char == key)) {
                    charCount.push(strObject[key]);
                }
            }
        }
        ...
        for (let l = 0; l < charCount.length - 1; l++) {
            if (charCount[l] == charCount[l + 1] &&
                pass[l] > pass[l + 1]) {
                continue test;
            } else if (charCount[l] < charCount[l + 1]) {
                continue test;
            }
        }

        validRoomIDs.push(ID);
        day04ArrTwo.push(currRoom.slice(0, -1));
    }

    return validRoomIDs.reduce((a, b) => a + b, 0);
}
...
function day04Two() {
    // const target = "northpoleobjectstorage";
    // for (let i = 0; i < day04ArrTwo.length; i++) {
    //     let sentence = "";
    //     for (let j = 0; j < day04ArrTwo[i].slice(0, -1).length; j++) {
    //         for (let k = 0; k < day04ArrTwo[i][j].length; k++) {
    //             let alphabet = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"];
    //             let index = alphabet.indexOf(day04ArrTwo[i][j][k])
    //             for (let l = 0; l < day04ArrTwo[i].slice(-1)[0]; l++) {
    ...
    //     if (sentence.slice(0, 4) == target.slice(0, 4)) {
    //         console.log(sentence)
    //         return day04ArrTwo[i].slice(-1)[0];
    //     }
    // }
    ...
    for (let i = 0; i < day04ArrTwo.length; i++) {
        let sentence = "";
        for (let k = 0; k < day04ArrTwo[i][0].length; k++) {
            let alphabet = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"];
            let index = alphabet.indexOf(day04ArrTwo[i][0][k])
            for (let l = 0; l < (day04ArrTwo[i].slice(-1)[0] % 26); l++) {
                alphabet.push(alphabet.shift())
            }
            sentence += alphabet[index];
        }

        if (sentence.slice(0, 5) == target) {
            return day04ArrTwo[i].slice(-1)[0];
        }
    }
}

console.log(`Part one's answer is: ${day04()}`);
console.log(`Part two's answer is: ${day04Two()}`);