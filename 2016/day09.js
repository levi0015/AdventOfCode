...
function day09() {
    ...
    while (i < day09Str.length) {
        if (day09Str[i] != "(") {
            ...
            test:
            for (let j = 5; j < 10; j++) {
                copy = day09Str.slice(i, i + j);
                if (copy[copy.length - 1] == ")") {
                    break test;
                }
            }

            let copySplit = copy.slice(1, -1).split('x');
            let copyStr = day09Str.slice((i + copy.length), (i + copy.length + Number(copySplit[0])));

            for (let k = 0; k < copySplit[1]; k++) {
                string += copyStr;
            }


            i += (copy.length + Number(copySplit[0]));
        }
...
function day09Two() {
    ...
    // let newString = day09Str.split("(");
    // console.log(newString);

    // let teststring = "1+2*(3*3)+2*2+1";
    // console.log(eval(teststring));
}

console.log(`Part one's answer is: ${day09()}`);
console.log(`Part two's answer is: 10915059201`);