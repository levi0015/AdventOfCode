...
const day08Arr = `dcga cadgbfe gecba cbfde eda cdbea gbadfe fegcba bedgca da | bgefdac bdace ad agcd
...
function day08(input, partTwo) {
    ...
    if (partTwo) {
        ...
        let testInp = input.split('\n').map(arr => arr.split(' | ').map(code => code.split(" ")))

        for (let i=0; i<testInp.length; i++) {
            for (let j=0; j<testInp[i][1].length; j++) {
                if (testInp[i][1][j].length == 2 || testInp[i][1][j].length == 3 || testInp[i][1][j].length == 4 ||testInp[i][1][j].length == 7) {
                    count++
                }
            }
        }
...
console.log(`Part one's answer is: 479`);
console.log(`Part two's answer is: 1041746`);