console.log("%cDay 13", 'color: #00ff44');

const day13Arr = `[[[[]]],[[[1,2,7,3,10],10,8,4,10]],[1,3,[],6,0]]
[[[[9,3,5],[5,9,1,2,9],8],[[],9]]]
...
function day13(input, partTwo) {
    input = input.split('\n\n').map(x => x.split('\n').filter(y => y != "").map(y => JSON.parse(y)));
    
    let answerOne;
    let answerTwo;

    answerOne = input.map((a, b) => {
        const isOrder = compare(a[0], a[1]) < 0;
        return [isOrder, b + 1];
    }).filter(x => x[0]).reduce((a, b) => a + b[1], 0);

    
    if (partTwo) {
        answerTwo = input.concat([[[[2]], [[6]]]]).flat().sort(compare).map((a, b) => [a, b + 1]).filter(x => JSON.stringify(x[0]) == '[[2]]' || JSON.stringify(x[0]) == '[[6]]').reduce((a, b) => a * b[1], 1);
    }
...
function compare(l, r) {
    ...
    if (!Number.isFinite(l) && !Number.isFinite(r)) {
        const shortLength = (l.length < r.length) ? l.length : r.length;
