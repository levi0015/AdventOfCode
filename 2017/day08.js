...
const day08Arr = `kd dec -37 if gm <= 9
x dec -715 if kjn == 0
ey inc 249 if x < 722
n dec 970 if t > 3
f dec -385 if msg > -3
...
function day08(input) {
    ...
    for (let i = 0; i < input.length; i++) {
        registers[input[i][0]] = 0;
        registers[input[i][4]] = 0;
    }

    for (let j = 0; j < input.length; j++) {
        let equation = eval(`${registers[input[j][4]]} ${input[j][5]} ${input[j][6]}`);
        if (equation) {
            if (input[j][1] == "inc") {
                registers[input[j][0]] += Number(input[j][2]);
            } else {
                registers[input[j][0]] -= Number(input[j][2]);
            }
        }
        let maxRoundValue = Math.max(...Object.values(registers));
        if (maxRoundValue > highestValue) {
            highestValue = maxRoundValue;
        }
    }

    return [(Math.max(...Object.values(registers))), highestValue];
}

console.log(`Part one's answer is: ${day08(day08Arr)[0]}`);
console.log(`Part two's answer is: ${day08(day08Arr)[1]}`);