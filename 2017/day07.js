...
function day07(input) {
        ...
        if (input[i].length >= 3) {
            ...
            for (let j = 2; j < input[i].length; j++) {
                if (j != input[i].length - 1) {
                    balancedTowers.push(input[i][j].slice(0, -1))
                } else {
                    balancedTowers.push(input[i][j])
                }
            }
        }
    }
    ...
    let startTower = (arr.filter(x => !balancedTowers.includes(x)));

    // console.timeEnd();
    return startTower[0];
}

function day07Two(input) {
    input = input.split('\n').map(str => str.split(' '));
    ...
    let startTower = (arr.filter(x => !balancedTowers.includes(x)));
    let allTowers = (input.filter(x => x[0] != startTower));

    // console.log(allTowers)
}

console.log(`Part one's answer is: ${day07(day07Arr)}`);
console.log(`Part two's answer is: ${day07Two(day07Arr)}`);