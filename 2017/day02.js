...
function day02(partTwo) {
    let checksum = 0;
    day02Arr.forEach((arr, i) => {
        if (partTwo) {
            arr.forEach((numOne, j) => {
                arr.forEach((numTwo, k) => {
                    if ((j != k) && (numOne % numTwo == 0)) {
                        checksum += (numOne / numTwo);
                    }
                });
            });
        } else {   
            checksum += (Math.max(...arr) - Math.min(...arr));
        }
    });
    return checksum;
}
...
console.log(`Part two's answer is: ${day02(true)}`);