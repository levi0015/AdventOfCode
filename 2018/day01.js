...
function day01Two() {
    ...
    for (let h = 0; h < iterations; h++) {
        for (let i = 0; i < day01Arr.length; i++) {
            const newFrq = frequencies[frequencies.length - 1] + day01Arr[i]
            // console.log(frequencies.length)
            for (let j = 0; j < frequencies.length; j++) {
                if (frequencies[j] == newFrq) {
                    // foundFrq = true;
                    return newFrq;
                }
            }
            
            frequencies.push(newFrq);
        }
...
console.log(`Part two's answer is: 390`);